            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="card-row">
                    @foreach ($listLelang as $item)
                        <a href="{{ route('details.show', $item->id_lelang) }}">
                            <div class="card">
                                <img src="/bids/{{ $item->lots }}" alt="{{ $item->barang->nama_barang }}">
                                <div class="card-status {{ $item->status == 'OPEN' ? 'OPEN' : 'CLOSED' }}">
                                    <h3>{{ $item->status }}</h3>
                                </div>
                                <div class="card-content">
                                    <h3>{{ $item->barang->nama_barang }}</h3>
                                    <h5>Auctioneer: <span style="color: red">{{ $item->barang->auctioneer }}</span></h5>
